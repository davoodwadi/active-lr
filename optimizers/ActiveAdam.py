...
class ActiveAdam(Optimizer):
    ...
    def __init__(self, params, stepSize, lr=1e-3, betas=(0.9, 0.999), eps=1e-8,
                 weight_decay=0, amsgrad=False, lrHigh=.2, lrLow=.5, delr=False):
        if not 0.0 <= lr:
        ...
        defaults = dict(lr=lr, betas=betas, eps=eps,
                        ...
                        lrHigh=lrHigh, lrLow=lrLow, stepSize=stepSize, delr=delr)
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                if state['step']/group['stepSize'] > 1 and state['step']%group['stepSize']==0:
                    ...
                    state['gai'] = torch.where(tmp2*tmp3<=0, tmp5*group['lrLow'], tmp5+group['lrHigh'])
                    gai = state['gai']
                ...
                if state['step']%group['stepSize']==0:
                    cumm = state['cumm']
                    # print(f"epoch complete. cumm: {cumm}")
